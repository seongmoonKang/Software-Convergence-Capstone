...
class BuildModel():
    ...
    def evaluate(self, epoch_i):
        ...
        with h5py.File('tvsum.h5') as new_file:
            with h5py.File(self.config.data_path) as data_file:
                for feature, label, index in tqdm(self.test_dataset, desc = 'Evaluate', ncols = 90, leave = False):
                    ...
                    new_info = new_file['video_'+str(index)]
                    pred_score, pred_selected, pred_summary = eval.select_keyshots(video_info, pred_score,new_info)
...
if __name__ == '__main__':
    ...
    from data_loader import get_loader
    train_config = Config()
    train_loader, test_dataset = get_loader(train_config.data_path, batch_size = 5)

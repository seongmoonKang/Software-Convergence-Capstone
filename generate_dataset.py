...
parser.add_argument('--vsumm_data', type=str,
                    ...
                    default='eccv16_dataset_summe_google_pool5.h5')
...
def video2fea(video_path, h5_f,vsumm_data):
    ...
    print("usr_sum_arr" , usr_sum_arr.shape)
    ...
    print('user summary 길이', len(usr_sum))
    print('length', length)
    while success:
        if (i + 1) % ratio == 0:
...
def make_dataset(video_dir, h5_path):
    ...
    with h5py.File(h5_path, 'w') as h5_f:
        for video_path in tqdm(video_list, desc='Video', ncols=80, leave=False):
            video2fea(video_path, h5_f,vsumm_data)

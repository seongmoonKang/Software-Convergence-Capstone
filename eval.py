...
def eval_metrics(y_pred, y_true):
    print("예측 중요 지점", sum(y_pred))
    print("사람 요약 지점(ground truth)", sum(y_true))
...
def select_keyshots(video_info, pred_score,new_info):
    ...
    print("최대 허용 시간", int(0.15*vidlen))
    ...
    print("실제 요약 영상 길이", sum(key_labels))
